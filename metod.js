...
function getAllNames(array) {
  const usernameAll = array.forEach((item) => {
...
function getUserById(array, id) {
  const getId = array.find((items) => items.id === id);
  console.log(getId);
}
...
function removeAddresses(array) {
  const addressDelete = array.map((item) => {
    delete item.address;
    return item;
  });
...
function deleteUser(array, id) {
  const DeleteId = array.filter((items) => items.id !== id);
  return DeleteId;
}
...
function getUsersGeo(array, id) {
  const getGeo = array.find((item) => item.id === id);
  return getGeo.address.geo;
}
